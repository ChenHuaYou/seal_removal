...
ICudaEngine* build_engine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name) {
    ...
    Int8EntropyCalibrator2* calibrator = new Int8EntropyCalibrator2(1, INPUT_W, INPUT_H, "./coco_calib/", "../yolov5m_int8calib.table", INPUT_BLOB_NAME);
...
int main(int argc, char** argv) {
    ...
    // ubuntu终端中运行
    // 序列化： ./yolov5 -s ../yolov5.wts ../yolov5m.engine m 0.17 0.25
    // 反序列化： ./yolov5 -d ../yolov5m.engine ../data
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            for (size_t j = 0; j < res.size(); j++) {
                ...
                // std::cout << "width = " << std::to_string(r.width) <<std::endl;
                // std::cout << "height = " << std::to_string(r.height) <<std::endl;
